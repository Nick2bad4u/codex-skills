...
For a raw endpoint, prefer a relative v3 path. Absolute URLs must match the configured HTTPS origin and API base path. The helper rejects URL credentials, fragments, traversal segments, token-like query parameters, and redirects; redacts sensitive JSON keys; limits cursor pagination; and retries Codacy's documented transient rate-limit responses conservatively.

## Codacy-Specific Gotchas

- The current v3 API uses cursor pagination. Continue until `pagination.cursor` is absent; do not stop after the first page.
- Codacy Cloud rate-limits by source IP. Treat HTTP 503 or 504 as potentially rate-limited, delay, and retry conservatively.
