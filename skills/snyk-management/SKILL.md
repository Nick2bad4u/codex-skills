...
- Tokens are region-specific. Configure the CLI environment and REST base URL for the account region; repeated `401` responses from a valid token often mean a region mismatch.
...
8. Verify asynchronously.
   Re-run the matching read or local scan. A queued test/import/monitor job is pending until its job endpoint or UI reaches a terminal state.
